...
//#define ADC_RANGE (bmSingleEnded | bmAdcRange_u2V5)
...
//#define ADC_RANGE (bmSingleEnded | bmAdcRange_b2V5)
...
int main(int argc, char **argv)
{
	...
	for (int channel = START_CHANNEL; channel <= END_CHANNEL; channel++)
		apci_write8(fd, 1, BAR_REGISTER, ofsAdcRange + channel, ADC_RANGE);
	...
	// int detectedMux;
	// apci_read32(fd, 1, BAR_REGISTER, 0xA4, &detectedMux);
	// printf("\nread +A4 == %02X\n", detectedMux);
	apci_write32(fd, 1, BAR_REGISTER, ofsSubMuxSelect, bmNoSubMux);
